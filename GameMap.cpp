...
public:
    ...
    Room(float x, float y, float z) : startx(x), starty(y), startz(z) {
    };
...
void StartAtRoom(int rid) {
    ...
    printf("Starting at %d, %f\n", rid, rooms[rid]->startx);
    gPositionX = rooms[rid]->startx;
    gPositionY = rooms[rid]->starty;
    gPositionZ = rooms[rid]->startz;
}
