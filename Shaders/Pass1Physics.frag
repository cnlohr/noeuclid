...
/*			vec3 ra = texture2D( AdditionalInformationMap, vec2( lastvox.b+(0./255.), lastvox.a ) ).rgb;
			...
			dir = dir * rm;
		//	AuxRotation = AuxRotation * rm;
			rdir = normalize( dir * lastmov.xyz );
			vec3 offset = texture2D( AdditionalInformationMap, vec2( lastvox.b+(3./255.), lastvox.a ) );
			vec3 pivot = texture2D( AdditionalInformationMap, vec2( lastvox.b+(4./255.), lastvox.a ) );
			vec3 lp = CameraOffset/msize + ptr + offset;
